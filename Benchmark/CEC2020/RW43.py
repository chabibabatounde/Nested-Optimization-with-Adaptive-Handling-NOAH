...
class RW43(Function):
    def __init__(self, dim):
        ...
        x_start[76:81] = 0.2  # P_c (active power setpoints)
        x_start[81:86] = 0.18  # Q_c (reactive power setpoints)
        ...
        domain = [
            ...
            *[(-0.3, 0.3) for _ in range(37)],
            # Frequency deviation: w ∈ [-0.05, 0.05]
            (-0.05, 0.05),
        ...
        self.der_buses = np.array([33, 34, 35, 36, 37])  # Buses 34-38 (0-indexed)

        # Droop coefficients
        self.m_p = 0.02  # Active power droop [pu/pu]
        self.m_q = 0.03  # Reactive power droop [pu/pu]

        print("✓ RC43: Reactive Power Loss Minimization (Droop Control)")
        print(f"  - Variables: {n_vars}")
        print(f"  - Network: 38 buses (islanded)")
        print(f"  - DERs: 5 controllable units with droop")
        print(f"  - Objective: Min [0.5×(∑Q_sp + ∑P_sp)]")
        print(f"  - Focus: **Reactive power + frequency support**")
    ...
    def _create_synthetic_network(self):
        ...
        # Active loads (radial decay from slack)
        for i in range(1, 33):
            decay = np.exp(-0.05 * i)
            self.P[i, 0] = np.random.uniform(0.08, 0.18) * decay
            self.P[i, 1] = 1.0
            self.P[i, 5] = 1.0  # V_ref
            self.P[i, 6] = 1.0  # n_p (constant power)

        # Reactive loads (more voltage-sensitive)
        for i in range(1, 33):
            decay = np.exp(-0.05 * i)
            self.Q[i, 0] = np.random.uniform(0.03, 0.12) * decay
            self.Q[i, 1] = 1.0
            self.Q[i, 5] = 1.0  # V_ref
            self.Q[i, 6] = 2.0  # n_q (constant impedance - voltage sensitive)

        # ===== SYNTHETIC LINE DATA =====
        # Format: [from_bus, to_bus, R, X, B/2]
    ...
    def build_ybus(self, frequency_deviation=0.0):
        """
        Build bus admittance matrix (Y-bus) with frequency effects
        """
        n_bus = 38
    ...
    def eval(self, variables_values):
        """
        Evaluate islanded microgrid droop control

        Objective: **Minimize reactive power losses**
        f = 0.5 × (∑Q_sp + ∑P_sp)

        Multi-objective emphasis on reactive power
        """
        ...
        V_slack = x[75]  # Slack voltage magnitude
        ...
        V[0] = V_slack + 0.0j  # Slack bus (ref)
        V[1:38] = V_238
        ...
        for idx, bus_idx in enumerate(self.der_buses):
            # Active power droop (frequency support)
            P_der[bus_idx] = P_c[idx] * (1.0 - w)

            # Reactive power droop (voltage support)
            Q_der[bus_idx] = Q_c[idx] * (1.0 - V_mag[bus_idx])

        # ===== LOAD POWER (Voltage-Dependent) =====
        P_load_coeff = self.P[:, 0]
        Q_load_coeff = self.Q[:, 0]

        # P_load = P₀ × (|V|/V_ref)^n_p
        P_load = P_load_coeff * (V_mag / V_ref) ** self.P[:, 6]

        # Q_load = Q₀ × (|V|/V_ref)^n_q  (more voltage-sensitive)
        ...
        # ===== OBJECTIVE FUNCTION =====
        # **RC43: Focus on minimizing reactive power losses**
        P_sp = P_available
        Q_sp = Q_available

        # Multi-objective with 0.5 weighting
        # Both reactive AND active power minimization
        objective = 0.5 * (np.sum(Q_sp) + np.sum(P_sp))

        # ===== CONSTRAINT PENALTIES =====

        # 1. Power balance violations
...
class RW43_Simplified(Function):
    """
    Simplified version focusing on reactive power optimization
    """

    def __init__(self, dim):
        """
        Simplified RC43: Reactive power loss minimization
        """
        n_vars = 86
        ...
        Function.__init__(
            ...
            max_dimension=n_vars, name="RW43_Simplified"
        )
        self.types(['CEC2020RW'])
    ...
    def eval(self, variables_values):
        """
        Simplified radial network evaluation

        Objective: 0.5 × (∑Q_sp + ∑P_sp)
        """
        x = np.array(variables_values, dtype=float).reshape(1, -1)[0]
        ...
        # ===== OBJECTIVE: RC43 (Reactive + Active) =====
        total_P_der = 0.0
        total_Q_der = 0.0

        for idx, bus in enumerate(self.der_buses):
            P_der = P_c[idx] * (1.0 - w)
            Q_der = Q_c[idx] * (1.0 - np.abs(V[bus]))
            total_P_der += P_der
            total_Q_der += Q_der

        total_P_load = np.sum(self.P_load)
        total_Q_load = np.sum(self.Q_load)

        P_sp = total_P_der - total_P_load
        Q_sp = total_Q_der - total_Q_load

        # **Multi-objective: 0.5×(Q_loss + P_loss)**
        objective = 0.5 * (Q_sp + P_sp)
